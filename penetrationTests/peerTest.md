...
| Images         | ![Default credential POST returns 200](./default-credentials.png) |
...
| Images         | ![Malicious injection](./malicious-injection.png) ![Unusable accounts](./unusable-account.png)                                         |
...
| Images         | ![User-facing stack trace](./user-facing-stack-trace.png)                                                |
...
| Images         | ![User enumeration](./user-enumeration-script.png)                                                                                                               |
...
| Images         | ![User-facing stack trace](./malformed-pagination-script-results.png)                                                                                                                                                                                        |
...
| Images         | ![Logged into admin account](./unchanged-admin.png) |
...
| Images         | ![Successful deletion script](./successful-franchise-deletion.png) ![No franchises](./all-franchises-deleted.png)                                       |
...
| Images         | ![Users enumerated](./user-enumeration.png)                                                                                                                                                             |
...
| Severity       | 3                                                                                             |
| Description    | API error responses exposed internal stack traces, revealing file paths and server internals. |
| Images         | ![User-facing stack trace](./stack-trace-exposed.png)                                         |
...
# Summary of shared learning

- The attack surface on even a simple project like JWT Pizza is expansive and takes an incredible amount of care to protect
- System packages are a sneaky and common way for vulnerabilites to make their way into a project (JWT logger)
  Client machines can never be fully trusted (source of truth for sensitive data should always be internal i.e. the cost of a pizza purchase)
- The information passed to clients should be completely minimal i.e. no stack traces of any kind, only the bare minimum needed to communicate efficiently with backend
- Open source projects are very vulnerable to attacks fueled by AI
- Simple attacks like checking default credentials are still possible
- How to use the simple and common features of Burp Suite (discovering paths, sending payloads, etc)
- Endpoints are the main entry point into a service and need sophisticated authentication, rate limiting, and other hardening features to avoid lots of common attacks
- Variables passed in requests should ideally come from the backend as much as is feasible. For example, menu items should be found in the database and sent rather than just through the client
